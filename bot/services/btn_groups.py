...
from db.models import GroupModel, PaymentStatus
...
async def get_my_groups(message: types.Message):
    ...
    for group_data in groups:
        await message.answer(
            text=get_localized_group(group_data),
            reply_markup=get_admin_group_keyboard(group_data.join_id)
        )


def get_localized_group(group) -> str:
    user_count = len(group.members)
    paid_count = len([member for member in group.members if dict(member).get('status') == PaymentStatus.PAID])
    ...
    return localize_text(
        ...
        name=group.name,
        date=group.next_payment_date.strftime('%d.%m'),
